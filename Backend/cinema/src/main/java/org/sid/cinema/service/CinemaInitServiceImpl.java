...
public class CinemaInitServiceImpl implements ICinemaInitService{
	...
	public void initCinemas() {
		villeRepository.findAll().forEach(v->{
			Stream.of("Megarama","Imax","Links","Chahrazad")
			.forEach(nameCinema->{
				Cinema cinema = new Cinema();
				cinema.setName(nameCinema);
				cinema.setNombrSalles(3+(int)(Math.random()*7));
				cinema.setVille(v);
	...
	public void initSalles() {
		cinemaRepository.findAll().forEach(cinema ->{
			for(int i=0;i<cinema.getNombrSalles();i++) {
	...
	public void initFilms() {
		...
		Stream.of("12 Hommes en colaire","Forest Gump","Green Book","La Ligne Verte","Le Parin","Le Seigneur des anneaux").forEach(f ->{
			Film film= new Film();
			film.setTitre(f);
